...
articles %>% 
  ...
  mutate(linea=trimws(linea)) %>%
  count(ano, linea) %>% 
  ggplot(aes(x=linea,y=ano, fill = n)) + # , color=source
  geom_tile() +
  coord_flip() +
  theme_minimal() +
  theme(axis.title.x = element_blank(), axis.title.y = element_blank())
...
esto deberia ser articulos x universidad (sin importar la cantidad de autores)

2DO: articulso -- contar universidad distintc() -- agrupar por año!!!
...
autores %>% 
  ...
  ggraph::ggraph(layout='fr') +
    geom_edge_link()+
    geom_node_point()+
    geom_node_text(aes(label = name, size=articulos), repel = TRUE) +
    theme_graph()
