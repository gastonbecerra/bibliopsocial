...
articles %>% 
  ...
  pivot_longer(cols = c(`MENTAL-HEALTH`, `POSITIVE-PSYCHOLOGY`, PERSONALITY, PREJUDICE, `POLITICAL-PSYCHOLOGY`, HISTORY, PSYCHOMETRICS, SOCIAL_SCIENCES, THEORETICAL, QUALITATIVE, QUANTITATIVE, BIBLIOMETRY),
               names_to = "topic",
               values_to = "present") %>%
  filter(present == TRUE) %>%  
  count(YEAR, topic) %>% 
...
articles %>% 
  select(-SAMPLE, -POPULATION) %>%  
  pivot_longer(cols = c(PSYCHOMETRICS, THEORETICAL, QUALITATIVE, QUANTITATIVE, BIBLIOMETRY),
               names_to = "topic",
               values_to = "present") %>%
  filter(present == TRUE) %>%  
  count(YEAR, topic) %>% 
